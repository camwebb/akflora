...
CREATE TABLE `wcsp_tmp` (
  ...
  `status` varchar(12) NOT NULL,
   -- enum('Accepted','Misapplied','Synonym','Unresolved') DEFAULT NULL,
  `accsyn` varchar(20) DEFAULT NULL
) ;

-- ALTER TABLE `wcsp_tmp` CHANGE `code` `code` VARCHAR(20) CHARACTER SET utf8 COLLATE utf8_bin NULL DEFAULT NULL;
...
UPDATE `wcsp_tmp` SET `md5sum` = MD5(concat_ws(' ',  `genhyb`, `genus`, `sphyb` , `species` , `ssptype` , `ssp` , `author`));
...
INSERT INTO `names` (`genhyb`, `genus`, `sphyb` , `species` , `ssptype` , `ssp` , `author`)
    SELECT DISTINCT `wcsp_tmp`.`genhyb`, `wcsp_tmp`.`genus`, `wcsp_tmp`.`sphyb` , `wcsp_tmp`.`species` , `wcsp_tmp`.`ssptype` , `wcsp_tmp`.`ssp` , `wcsp_tmp`.`author`
    FROM `wcsp_tmp` LEFT JOIN `names` AS `A` ON  `wcsp_tmp`.`md5sum` = `A`.`md5sum` WHERE `A`.`md5sum` IS NULL ;

-- Records: 179077  Duplicates: 0  Warnings: 0
