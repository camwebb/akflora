...
BEGIN{
  ...
  # see error messages at bottom of script that cause me to drop these
  # NB, the main DB cannot accept hybrids of the form G sp1 × sp2
}

(!kill[$1]) {
  # Clean trailing spaces in names
  gsub(/ +$/,"",$2)
  gsub(/ +$/,"",$5)
  ...
  #   these are synonym (5) not an accepted (1)
  if (fix5[$1]) $3 = 5
  # one case of an accepted being a syn
  if (fix1[$1]) $3 = 1
  ...
  # So ... if accepted, but the names are diff (and not errors, as above)
  if (($3 == 1) && ($2 != $5)) {
    # re-write the adjudicated name, overwriting the original adjudicated name
    print "swapped: " $1, $2, $5 > "/dev/stderr"
    $2 = $5
  }

  # 2020-01-17: 3 were swapped:
  # swapped: 4647|Elymus violaceus (Hornem.) Böcher ex J. Feilberg|\
  #  Elymus violaceus (Hornem.) Feilberg
  # swapped: 5451|Glyceria maxima (Hartm.) Holmb.|\
  #  Glyceria maxima (Hartm.) Holumb.
  # swapped: 10001|Protopannaria pezizoides (Weber) P.M. Jørg. & S. Ekman|\
  #  Protopannaria pezizoides (Weber) P.M. Joerg. & S. Ekman

  # test that each species string occurs only once
  if (++c2[$2] > 1) print $2 " (adjID = " $1 " ) is a dup " c2[$2]  \
                      > "/dev/stderr"

  #OK, time to process, using the name+author string as a key
...
END{
  ...
  # main: first parse the names, using the unparsed names as keys
  # for each name string in the adjudicated table
  for (i in id_of_name) {
    # parse it (uses external library)
  ...
  for (i in id_of_name)
    ...
    else print "Error: " i " not listed" > "/dev/stderr"

  # Finally, this would not parse using parse_taxon_name(), so add directly:
  print "accs-12673||Uva-Ursi||uva-ursi|||(L.) Britton|synonym|accs-1051|Panarctic Flora Checklist"
  
}

# Some error messages for names that would not parse:
#
