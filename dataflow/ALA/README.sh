# Making the base ALA checklist, Cam Webb, 2018-11-08 onwards
#   The input file (DFMAccepNameswLit20180609B.TXT) was provided by Al Batten
#   as a dump from the 4D database that he and Dave Murray had developed for
#   20+ years.

# 0. Prepare

# mv DFMAccepNameswLit20180609B.TXT ALA_in.TXT
rm -f ala.*

# 1. Character encoding, cleaning, reformating

# Base file: line-endings: Mac (\n) , delimiters = "\t", columns:
...
iconv -f MACINTOSH -t UTF-8 ALA_in.TXT | \
    tr "\r" "\n" | tr "\t" "|" | tail -n +2 > ala.1
...
gawk -f split_names.awk ala.6 > ala.7
